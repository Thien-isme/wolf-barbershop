...
// Thêm hàm kiểm tra ngày có hợp lệ
const disabledDate = (current: dayjs.Dayjs) => {
    const today = dayjs().startOf('day');
    const threeDaysFromNow = today.add(2, 'day').endOf('day');
    return current.isBefore(today) || current.isAfter(threeDaysFromNow);
};
...
const DateTimeSelect = ({
    ...
    // Đặt hàm isTimeSlotPassed vào đây để có thể truy cập selectedDate
    const isTimeSlotPassed = (timeSlot: string) => {
        ...
        // Chỉ check các khung giờ trong ngày hiện tại
        if (!selectedDate.isSame(today, 'day')) return false;

        // Chuyển đổi timeSlot (VD: "09:00") sang dayjs object
        const [hours, minutes] = timeSlot.split(':').map(Number);
        const slotTime = dayjs().set('hour', hours).set('minute', minutes);
            ...
            >
                <Typography.Title level={5} style={{ color: 'white', marginBottom: 8 }}>
                    ...
                    >
                        {selectedDate
                            ? `(Thứ ${
                                  selectedDate.day() === 0 ? 8 : selectedDate.day() + 1
                    ...
                    >
                        ...
                        >
                            {timeSlots.map(slot => {
                                ...
                                const isDisabled =
                                    disabledTimes.includes(slot) || isPassed;
                                ...
                                return (
                                    <Button
                                        ...
                                        type='default'
                                        onClick={() =>
                                            !isDisabled && setSelectedTime(slot)
                                        }
                                        disabled={isDisabled}
                                        style={{
